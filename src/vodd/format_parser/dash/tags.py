...
from .attribute_parsers import get_bool_value, get_float_value, get_int_value, get_list_of_type, organize_ns

LOOKUP_STR_FORMAT = './*[local-name(.) = "{target}" ]'
KEYS_NOT_FOR_SETTING = ['element', 'tag_map', 'encoding']

DRMSolution = (
    _ := namedtuple('DRMSolution', field_names=[
        'widevine',
        'playready',
        'cenc',
    ])
)(*_._fields)
DRM_SOLUTIONS = DRMSolution._fields


def get_base_uri(url: str) -> (str, None):
    if not url:
        return None
    parse = urlparse(url)
    base_uri = f"{parse.scheme}://{parse.netloc}{parse.path.rsplit('/', 1)[0] + '/'}"
    return base_uri


def get_segment_url(segment_url: str, base_uri: str) -> str:
    """
    拼接切片链接
    """
    if segment_url.startswith('http') or base_uri is None:
        return segment_url
    if segment_url.startswith('//'):
        segment_url = f'https:{segment_url}'
    elif segment_url.startswith('/'):
        parse = urlparse(base_uri)
        segment_url = f'{parse.scheme}://{parse.netloc}{segment_url}'
    else:
        segment_url = f'{base_uri}{segment_url}'
    return segment_url


def replace_segment_name(template: str, **kwargs):
    if 'representation_id' in kwargs:
        template = template.replace('$RepresentationID$', str(kwargs['representation_id']))
    if 'time' in kwargs:
        template = template.replace('$Time$', str(kwargs['time']))
    if 'bandwidth' in kwargs:
        template = template.replace('$Bandwidth$', str(kwargs['bandwidth']))
    if 'number' in kwargs:
        template = template.replace('$Number$', str(kwargs['number']))
    return template


def get_mpd_attr(parent, value, tag):
    while not value and hasattr(parent, 'parent'):
        if hasattr(parent := getattr(parent, 'parent'), tag):
            value = getattr(parent, tag)
    return value
...
class ContentProtection(Tag):
    ...
    def __init__(self, element):
        ...
        self._solutions = {
            'urn:uuid:edef8ba9-79d6-4ace-a3c8-27dcd51d21ed': DRMSolution.widevine,
            'urn:uuid:9a04f079-9840-4286-ab92-e65be0885f95': DRMSolution.playready,
            'urn:mpeg:dash:mp4protection:2011': DRMSolution.cenc
        }

    @cached_property
    def solution(self):
        return self._solutions.get(self.scheme_id_uri)
    ...
    @cached_property
    def drm(self):
        return {member.prefix: member.text for member in self.element}
...
class RepresentationBase(Tag):  # pylint: disable=too-many-public-methods
    ...
    def mime_type(self):
        return get_mpd_attr(
            self,
            self.element.attrib.get('mimeType'),
            'mime_type'
        )
    ...
    def codecs(self):
        return get_mpd_attr(
            self,
            self.element.attrib.get('codecs'),
            'codecs'
        )
    ...
    @cached_property
    def content_protections(self) -> List[ContentProtection]:
        return get_mpd_attr(
            self,
            [ContentProtection(member) for member in
...
class AdaptationSet(RepresentationBase):  # pylint: disable=too-many-public-methods
    ...
    def __init__(self, element, period: 'Period'):
        super().__init__(element)
        self.parent = period
        self.tag_map = {
    ...
    def base_urls(self):
        return get_mpd_attr(
            self,
            [BaseURL(member) for member in self.element.xpath(LOOKUP_STR_FORMAT.format(target="BaseURL"))],
            'base_urls'
        )
    ...
    def representations(self):
        return [Representation(member, self) for member in
...
class Representation(RepresentationBase):
    ...
    def __init__(self, element, adaptation_set: AdaptationSet):
        super().__init__(element)
        self.parent = adaptation_set
    ...
    @cached_property
    def base_urls(self) -> List['BaseURL']:
        return get_mpd_attr(
            self,
            [BaseURL(member) for member in self.element.xpath(LOOKUP_STR_FORMAT.format(target="BaseURL"))],
            'base_urls'
    ...
    @cached_property
    def segments(self):
        segment_template = self.segment_templates[0]
        if '$Number$' in segment_template.media:
            yield from self.get_number_segments()
        elif '$Time$' in segment_template.media:
            yield from self.get_time_segments()

    def get_number_segments(self):
        mpd = self.parent.parent.parent
        media_presentation_duration = parse_duration(mpd.media_presentation_duration).total_seconds()
        base_url = self.base_urls[0].text
        segment_template = self.segment_templates[0]
        if (timescale := segment_template.timescale) is None:
            timescale = self.parent.segment_templates[0].timescale
        start_number = segment_template.start_number
        duration = float(segment_template.duration / timescale)
        sequence_nos = set()
        segments = []
        for i in range(math.ceil(media_presentation_duration / duration)):
            sequence_no = start_number + i
            if sequence_no in sequence_nos:
                continue
            sequence_nos.add(sequence_no)
            segment_url = get_segment_url(
                f'{base_url}{replace_segment_name(segment_template.media, representation_id=self.id, bandwidth=self.bandwidth, number=sequence_no)}',
                self.parent.parent.parent.base_uri
            )
            if segment_template.initialization:
                initialization_url = get_segment_url(
                    f'{base_url}{replace_segment_name(segment_template.initialization, representation_id=self.id, bandwidth=self.bandwidth, number=sequence_no)}',
                    self.parent.parent.parent.base_uri
                )
            else:
        ...
        return sorted(segments, key=lambda x: x['sequence_no'])
    ...
    def get_time_segments(self):
        ...
        sequence_nos = set()
        segments = []
        for s in segment_template.segment_timelines[0].base_urls:
            if s.t:
                t = s.t
            for i in range((s.r or 0) + 1):
                if t in sequence_nos:
                    continue
                sequence_nos.add(t)
                segment_url = get_segment_url(
                    f'{base_url}{replace_segment_name(segment_template.media, representation_id=self.id, time=t)}',
                    self.parent.parent.parent.base_uri
                )
                if segment_template.initialization:
                    initialization_url = get_segment_url(
                        f'{base_url}{replace_segment_name(segment_template.initialization, representation_id=self.id, time=t)}',
                        self.parent.parent.parent.base_uri
                    )
                else:
                    initialization_url = None
                segments.append({
                    'compare_no': (t - segment_template.presentation_time_offset) / timescale,
                    'sequence_no': t,
                    'duration': str(s.d / timescale),
                    'segment_url': segment_url,
                    'initialization_url': initialization_url,
                })
                t += s.d
        return sorted(segments, key=lambda x: x['compare_no'])
    ...
    @cached_property
    def segment_templates(self) -> List['SegmentTemplate']:
        return get_mpd_attr(
            self,
            [SegmentTemplate(member) for member in
             self.element.xpath(LOOKUP_STR_FORMAT.format(target="SegmentTemplate"))],
            'segment_templates'
        )
...
class SegmentBase(Tag):
    ...
    def presentation_time_offset(self):
        return get_int_value(self.element.attrib.get('presentationTimeOffset')) or 0
...
class Period(Tag):
    ...
    def __init__(self, element, mpd: 'MPD'):
        super().__init__(element)
        self.parent = mpd
    ...
    def adaptation_sets(self):
        return [AdaptationSet(member, self) for member in
...
class MPD(Tag):  # pylint: disable=too-many-public-methods
    ...
    def __init__(self, element: Element, url: str = None, encoding: str = "utf-8"):
        super().__init__(element=element)
        self.base_uri = get_base_uri(url)
    ...
    @cached_property
    def base_urls(self):
        if not (bus := [BaseURL(member) for member in self.element.xpath(LOOKUP_STR_FORMAT.format(target="BaseURL"))]):
            (elem := Element('BaseURL')).text = self.base_uri
            bus.append(BaseURL(elem))
        return bus
    ...
    @cached_property
    def periods(self):
        return [Period(member, self) for member in self.element.xpath(LOOKUP_STR_FORMAT.format(target="Period"))]
